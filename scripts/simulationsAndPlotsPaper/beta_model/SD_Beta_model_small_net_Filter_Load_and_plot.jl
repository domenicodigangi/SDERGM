...
using JLD, StaNets,  HelperFunDom
...
T = size(dynDegsSam.s)[2]
  for s=1:Nsample
    for t=1:T
          estFitSS.s[:,t,s] =  StaNets.identify(StaNets.NetModelDirBin1(dynDegsSam.s[:,t,s]),
                                                estFitSS.s[:,t,s];idType = "firstZero" )
          filFitGas.s[:,t,s] =  StaNets.identify(StaNets.NetModelDirBin1(dynDegsSam.s[:,t,s]),
