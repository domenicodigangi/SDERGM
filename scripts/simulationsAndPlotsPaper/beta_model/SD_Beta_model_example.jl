...
gasFiltFit,~ = DynNets.gasFilter(modGasDirBin1_3Npars,[parGas_3Npars[1];parGas_3Npars[2];parGas_3Npars[3]])
...
#f(x) = DynNets.gasFilter(modGasDirBin1_N_p_2, x;groupsInds = modGasDirBin1_N_p_2.groupsInds)[2]
GBA = 1
...
ftot_T_est, _ = gasFilter(modGasDirBin1_N_p_2, allpar0,ftotIO_0 = ftotIO_0)
ftot_T, _ = gasFilter(modGasDirBin1_N_p_2, allpar_sample[:,3],  ftotIO_0 = ftotIO_0)
plot(ftot_T_est')
#plot(ftot_T')
...
global gasFiltPar_conf = zeros(N_samp_par, Nterms, T_train)
      for i= 1:N_samp_par
       # @show(i)
       global gasFiltPar_conf[i,:,:], _ = gasFilter(modGasDirBin1_N_p_2, allpar_sample[:,i],  ftotIO_0 = ftotIO_0)
      end
