...
function samplSingMatCan(Model::NetModelDirBin1,expMat::Array{<:Real,2})
    """
    given the expected matrix sample one ranHelperFunDom matrix from the corresponding pdf
    """
    N1,N2 = size(expMat)
    N1!=N2 ? error() : N=N1
    out = zeros(Int8,N,N)
...
function expMatrix2(Model::NetModelDirBin1, parNodesIO::Array{<:Real,1}  )
    ...
    parMat =  HelperFunDom.putZeroDiag_no_mut(exp.(parI  .+ parO') )
...
function dgpDynamic(model::NetModelDirBin1,mode,N::Int,T::Int;NTV = 2,degIOUncMeans::Array{Float64,1} =zeros(10),
    ...
    #shuffle ranHelperFunDomly the fases of the sines but fix the ranHelperFunDom seed for reproducibility
    ...
    elseif uppercase(mode) ==  uppercase("steps")
        ...
        for n=1:NTV
            ...
            #circyularly shift time index by the ranHelperFunDom phase of each
            if false
                #circyularly shift time index by the ranHelperFunDom phase of each
