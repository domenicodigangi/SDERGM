...
GasNetModelDirBinGlobalPseudo( obsT , Par   ) =
        GasNetModelDirBinGlobalPseudo( obsT,Par ,trues(length(Par[:,1])) ,"")
#Initialize by observations only
GasNetModelDirBinGlobalPseudo(obsT::  Array{Array{Real,2}}) =   (Npar =  length(obsT[1][1,:]) - 2;
                                                GasNetModelDirBinGlobalPseudo(obsT,zeros(Npar,3) ))

GasNetModelDirBinGlobalPseudo(obsT:: Array{Real,3},scoreScalingType::String) =   (N = round(length(obsT[:,1])/2);
                                                GasNetModelDirBinGlobalPseudo(obsT,zeros(3Npar),scoreScalingType) )
...
function array2VecGasPar(Model::GasNetModelDirBinGlobalPseudo,
                            ArrayGasPar,
                            indTvPar :: BitArray{1})
...
#
# function unRestrictGasPar( Model::GasNetModelDirBinGlobalPseudo,vecReGasPar::Array{<:Real,1})
#      NTvPar = round(Int,length(vecReGasPar)/3)
#      W_Re = vecReGasPar[1:3:3*NTvPar]
#      diag_B_Re = vecReGasPar[2:3:3*NTvPar]
#      diag_A_Re = vecReGasPar[3:3:3*NTvPar]
#
#      W_Un = W_Re
#      diag_B_Un = log.(diag_B_Re ./ (1 .- diag_B_Re ))
#      diag_A_Un = log.(diag_A_Re)
#
#      vecUnPar = zeros(Real, size(vecReGasPar))
#      vecUnGasPar[1:3:3*NTvPar] = W_Un
#      vecUnGasPar[2:3:3*NTvPar] = diag_B_Un
#      vecUnGasPar[3:3:3*NTvPar] = diag_A_Un
#      return vecUnPar
#      end
...
function gasFilter( Model::GasNetModelDirBinGlobalPseudo,
...
gasScoreSeries(Model::GasNetModelDirBinGlobalPseudo,
                                  vStaticPar::Array{<:Real,1};obsT:: Array{Array{Float64,2},1}=Model.obsT) =
              gasScoreSeries(Model,repeat(vStaticPar,outer = (1,length(obsT)) );obsT= obsT)
...
function estimate(Model::GasNetModelDirBinGlobalPseudo;indTvPar::BitArray{1}=trues(length(Model.obsT[1][1,:])-2),
                    indTargPar::BitArray{1} = indTvPar, UM :: Array{<:Real,1} = zeros(length(Model.obsT[1][1,:])-2),
                    ftot_0 :: Array{<:Real,1} = zeros(length(Model.obsT[1][1,:])-2),changeStats_T = Model.obsT,
                    hess_opt_flag = false)
    "Estimate the GAS and static parameters of the Pseudolikelihood Score Driven ERGM "
...
function conf_bands_par_unc(Model::GasNetModelDirBinGlobalPseudo,
    N::Int,vResGasPar::Array{<:Real,1}, indTvPar::BitArray{1};
    vConstPar ::Array{<:Real,1} = zeros(Real,2),obsT:: Array{Array{Float64,2},1}=Model.obsT)

    return distr_fVecT
end

function conf_bands_filt_unc(Model::GasNetModelDirBinGlobalPseudo,
    N::Int,vResGasPar::Array{<:Real,1}, indTvPar::BitArray{1};
    vConstPar ::Array{<:Real,1} = zeros(Real,2),obsT:: Array{Array{Float64,2},1}=Model.obsT)
...
#
# function estimate(Model::GasNetModelDirBinGlobalPseudo;indTvPar::BitArray{1}=trues(length(Model.obsT[1][1,:])-2),
#                     indTargPar::BitArray{1}=falses(length(Model.obsT[1][1,:])-2), UM :: Array{<:Real,1} = zeros(length(Model.obsT[1][1,:])-2))
#     "Estimate the GAS and static parameters of the Pseudolikelihood Score Driven ERGM "
#     # se targeting è vero allora usa le start Um com target
#
#     #check that targeting is required only for time varying Parameters
...
#
#     # #set the starting points for the optimizations
#     B0_Re  = 0.8; B0_Un = log(B0_Re ./ (1-B0_Re ))
#     A0_Re  = 0.01; A0_Un = log(A0_Re)
#     vParAll0_Un = zeros(NergmPar + NTvPar*2 );last = 0
#     for i=1:NergmPar
#         if indTvPar[i]
#             if indTargPar[i]
#                 vParAll0_Un[last+1:last+2] = [ B0_Un; A0_Un]
#                 last+=2
#             else
#                 vParAll0_Un[last+1:last+3] = [startUM[i]*(1-B0_Re) ; B0_Un; A0_Un]
#                 last+=3
#             end
#         else
#             vParAll0_Un[last+1] = startUM[i]
#             last+=1
#         end
#     end
#     println(vParAll0_Un)
#     function divideParVec(vecPar::Array{<:Real,1})# a function of the groups parameters
#         # assuming that the vector is created by vertically stacking all parameters (3 for TV and 1 for static)
#         # divide the gas parameters in 2 vectors
#         vecGasPar = zeros(Real,3*NTvPar)
#         vecConstPar = zeros(Real,NergmPar-NTvPar)
#         lastTv,lastCon,lastAll = 0,0,0
#         for i=1:NergmPar
#
#             if indTvPar[i]
#                 if indTargPar[i]
#                     vecGasPar[lastTv+1:lastTv+2] = vecPar[lastAll+1:lastAll+2 ]
#                     lastAll += 2
#                     lastTv  += 2
#                 else
#                     vecGasPar[lastTv+1:lastTv+3] = vecPar[lastAll+1:lastAll+3 ]
#                     lastAll += 3
#                     lastTv  += 3
#                 end
#             else
#                 vecConstPar[lastCon+1] = vecPar[lastAll+1]
#                 lastAll +=1
#                 lastCon +=1
#             end
#         end
#         return vecGasPar,vecConstPar
#     end
#     # objective function for the optimization
#     function objfunGas(vecUnPar::Array{<:Real,1})# a function of the groups parameters
#         vecUnGasPar,vecConstPar =  divideParVec(vecUnPar)
...
#         #println(vecUnGasPar)
#         #println(vecConstPar)
#         Ntarg = sum(indTargPar)
#         vecReGasParAll = zeros(Real,length(vecUnGasPar)+Ntarg)
#         # add w determined by B values to targeted parameters
#         last = 0
#         lastAll = 0
#         for i=1:NTvPar
#             if indTargPar[indTvPar][i]
#                 B =  1 ./ (1+exp.(-vecUnGasPar[last+1]))
#                 vecReGasParAll[lastAll+1] = startUM[indTvPar][i]*(1-B)
#                 vecReGasParAll[lastAll+2] = B
#                 vecReGasParAll[lastAll+3] = exp(vecUnGasPar[last+2])
#                 last+=2
#             else
#                 vecReGasParAll[lastAll+1] = vecUnGasPar[last+1]
#                 vecReGasParAll[lastAll+2] =  1 ./ (1+exp.(-vecUnGasPar[last+2]))
#                 vecReGasParAll[lastAll+3] = exp(vecUnGasPar[last+3])
#                 last+=3
#             end
#             lastAll +=3
#             end
#         foo,loglikelValue = gasFilter(Model,vecReGasParAll,indTvPar;
#                                         obsT = changeStats_T,vConstPar =  vecConstPar)
#         #println(vecReGasPar)
#          return - loglikelValue
#     end
#     #Run the optimization
...
#         ADobjfunGas = objfunGas
#     else
#         ADobjfunGas = TwiceDifferentiable(objfunGas, vParAll0_Un; autodiff = :forward);
#     end
#
#      println(objfunGas(vParAll0_Un))
#      #error()
#     optim_out2  = optimize(ADobjfunGas,vParAll0_Un ,algo,optims_opt)
#     outParAll = Optim.minimizer(optim_out2)
#     println(outParAll)
#
#     arrayAllParHat_Un = vec2ArrayGasPar(Model,outParAll ,indTvPar,indTargPar;targVal = startUM)
#     arrayAllParHat = arrayAllParHat_Un
#     println(arrayAllParHat)
#     for i=1:NergmPar
#         if indTvPar[i]
#             arrayAllParHat[i] = restrictGasPar(Model,arrayAllParHat_Un[i])
#         end
#     end
#
#     conv_flag =  Optim.converged(optim_out2)
#     # println(optim_out2)
#     return  arrayAllParHat, conv_flag
# end
